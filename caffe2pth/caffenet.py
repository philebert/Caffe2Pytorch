...
class Eltwise(nn.Module):
    ...
    def __repr__(self):
        return 'Eltwise(operation=\'%s\')' % self.operation
...
class Softmax(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.softmax(x.view(-1, dims), dim=self.axis)
        x = x.view(*orig_size)
        return x
...
class Normalize(nn.Module):
    ...
    def __repr__(self):
        return 'Normalize(n_channels=%d, scale=%f)' % (self.n_channels, self.scale)
        ...
        while i < layer_num:
            ...
            if ltype in ['Convolution', 'Deconvolution']:
                ...
                if type(self.models[lname]) == nn.Sequential:
                    reshape_np = np.array(lmap[lname].blobs[0].data).reshape(self.models[lname][1].weight.data.shape)
                    self.models[lname][1].weight.data.copy_(torch.from_numpy(reshape_np))
                    if len(lmap[lname].blobs) > 1:
                        reshape_np = np.array(lmap[lname].blobs[1].data).reshape(self.models[lname][1].bias.data.shape)
                        self.models[lname][1].bias.data.copy_(torch.from_numpy(reshape_np))
                else:
                    reshape_np = np.array(lmap[lname].blobs[0].data).reshape(self.models[lname].weight.data.shape)
                    self.models[lname].weight.data.copy_(torch.from_numpy(reshape_np))
                    if len(lmap[lname].blobs) > 1:
                        reshape_np = np.array(lmap[lname].blobs[1].data).reshape(self.models[lname].bias.data.shape)
                        self.models[lname].bias.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data)))
                i = i + 1
            else:
